
//Interfaces
#include "FS.h"
#include <SPI.h>
#include <Wire.h>

//Sensors
#include <VL53L0X.h>
#include <Adafruit_BMP085.h>
#include <DS3231.h> // real time clock
#include <SD.h> 

// Multi app
// #include "SD_Card.h"
#include "distance_sensor.h"
#include "bmp180.h"

// Internet
// #include "web.h"
#include <WiFi.h>              //Built-in
#include <ESP32WebServer.h>    //https://github.com/Pedroalbuquerque/ESP32WebServer download and place in your Libraries folder
#include <ESPmDNS.h>
#include "CSS.h"
#include "front.h"

/* SERVER */
ESP32WebServer server(80);
#define servername "Float"
#define SD_pin 5 //G16 in my case
bool   SD_present = false; //Controls if the SD card is present or not
// #define w_mode 4 //G16 - Work Mode: logger or webserver

// Variables
int num = 0;
bool century = false;
bool h12Flag;
bool pmFlag;
int sleepTime = 30; 
const char* ssid = "WitFloat";
// const char* password = "123456789";
char buf[32];
char XML[2048];
bool logData = false;
bool motorOn = false;
double temp = 0;
double pressure = 0;
double depth = 0;
int dirPin = 14;
int stepPin = 4;
int enablePin = 13;
bool MotorToggle = 0;
const int analogPin = 34;
float inputP1, voltage, baselineV = 0.5, extPressure;
int count = 0;
bool MotorCCW_Switch = 0;
bool MotorCW_Switch = 0;
bool MotorAll_Disable = 0;

TaskHandle_t Task1;
// TaskHandle_t Task2;

// Objects
String data;
String timeNow;
DS3231 rtc;
String dataString = ""; //For saving data
String fileName = "DATALOG.TXT"; //File name to save the data

void setup(){
  Serial.begin(115200);
  // pinMode(w_mode, INPUT); //working mode Pin
  pinMode(dirPin, OUTPUT);
  pinMode(stepPin, OUTPUT);
  pinMode(enablePin, OUTPUT);
  //SD
  xTaskCreatePinnedToCore(MotorCode, "Task1", 10000, NULL, 0, &Task1, 0);
  // xTaskCreatePinnedToCore(SensorCode, "Task2", 10000, NULL, 0, &Task2, 0);
  Serial.print(F("Initializing SD card..."));
  
  //see if the card is present and can be initialised.
  //Note: Using the ESP32 and SD_Card readers requires a 1K to 4K7 pull-up to 3v3 on the MISO line, otherwise may not work
  if (!SD.begin(SD_pin))
  { 
    Serial.println(F("Card failed or not present, no SD Card data logging possible..."));
    SD_present = false; 
  } 
  else
  {
    Serial.println(F("Card initialised... file access enabled..."));
    SD_present = true; 
  } 
  // sd_setup();
  // web_setup();
  // uint64_t cardSize = SD.cardSize() / (1024 * 1024);
  // Serial.printf("SD Card Size: %lluMB\n", cardSize);
  dist_setup();
  bmp_setup();
  // writeFile(SD, "/data.txt", "Time(s), Temp (C); Pressure (Pa); Depth(m)\n");
  num = 0;

    //If working mode equal to Webserver, reed switch on
  // if (digitalRead(w_mode)==1)
  // {
    //Server    
    WiFi.softAP(ssid); //Network and password for the access point genereted by ESP32
    
    //Set your preferred server name, if you use "mcserver" the address would be http://mcserver.local/
    if (!MDNS.begin(servername)) 
    {          
      Serial.println(F("Error setting up MDNS responder!")); 
      ESP.restart(); 
    } 
  
    /*********  Server Commands  **********/
    server.on("/",         SD_dir);
    server.on("/upload",   File_Upload);
    server.on("/fupload",  HTTP_POST,[](){ server.send(200);}, handleFileUpload);
    server.on("/datalog",  Data_log);
    server.on("/xml",      SendXML);
    server.on("/motortoggle",  Motor_toggle);
    server.on("/CW",  Motor_CW);
    server.on("/CCW",  Motor_CCW);
    server.on("/MotorDisable",  Motor_Disable);
  
    server.begin();
    
    Serial.println("HTTP server started");
    
  // }
}

void loop(){
  //Server
  
  server.handleClient(); //Listen for client connections
  // Serial.println(digitalRead(w_mode));
  
  if(logData)
  {
    // dist_loop();
    temp = bmp.readTemperature();
    pressure = bmp.readPressure();
    depth = calc_depth(pressure); 
    timeNow = {String(rtc.getHour(h12Flag, pmFlag)) + ":" + String(rtc.getMinute())+ ":" + String(rtc.getSecond())};
    inputP1 = analogRead(analogPin);
    // voltage = inputP1 *(5.0/4095.0);
    // baselineV = 0.5;
    // extPressure = (voltage - baselineV) * (100.0 / 3.3 - baselineV);
    // float outputP1 = ((float)inputP1/1024*100);
    Serial.print("Sensor Analog: ");
    Serial.print(inputP1);
    // Serial.print("\nVoltage Analog: ");
    // Serial.print(voltage);
    // Serial.print("\nPressure Analog: ");
    // Serial.print(extPressure);
    Serial.print("\nTime: ");
    Serial.println(timeNow);
    data = {String(timeNow) + ", " + String(temp) + ", " + String(pressure) + ", " + String(depth) + ", " + String(inputP1) + "\r\n"};
    File dataF = SD.open("/dataLogged.txt", FILE_APPEND);
    dataF.print(data);
    // Serial.println(num);
    bmp_loop();
    
    // appendFile(SD, "/data.txt", data.c_str());
    delay(1000); 
  }
  // if (MotorToggle){
  //   if(count < 20){
  //     motorCW();
  //   } else if(count > 20 && count < 50) {
  //     motorDisable();
  //   } else {
  //     motorCCW();
  //   }
  //   count ++;
  //   Serial.print("Count:");
  //   Serial.print(count);
  // } else {
  //   count = 0;
  // }
  
}

void MotorCode(void *parameter){
  for(;;){
    if (MotorToggle){
      if(count < 10000){
        motorCCW();
      } else if(count >= 10000 && count < 20000) {
        motorDisable();
      } else if(count >= 20000 && count < 30000){
        motorCW();
      } else {
        MotorToggle = !MotorToggle;
      }
      count ++;
      Serial.print("\nCount:");
      Serial.print(count);
    } else {
      count = 0;
    }
    if(MotorCCW_Switch){
      motorCCW();
    }
    if(MotorCW_Switch){
      motorCW();
    }
    if(MotorAll_Disable){
      motorDisable();
    }
  }
  vTaskDelete(Task1);
}

// void SensorCode(void *parameter){
//   for(;;){
//     if(logData)
//     {
//       // dist_loop();
//       temp = bmp.readTemperature();
//       pressure = bmp.readPressure();
//       depth = calc_depth(pressure); 
//       timeNow = {String(rtc.getHour(h12Flag, pmFlag)) + ":" + String(rtc.getMinute())+ ":" + String(rtc.getSecond())};
//       inputP1 = analogRead(analogPin);
//       // voltage = inputP1 *(5.0/4095.0);
//       // baselineV = 0.5;
//       // extPressure = (voltage - baselineV) * (100.0 / 3.3 - baselineV);
//       // float outputP1 = ((float)inputP1/1024*100);
//       Serial.print("Sensor Analog: ");
//       Serial.print(inputP1);
//       // Serial.print("\nVoltage Analog: ");
//       // Serial.print(voltage);
//       // Serial.print("\nPressure Analog: ");
//       // Serial.print(extPressure);
//       Serial.print("\nTime: ");
//       Serial.println(timeNow);
//       data = {String(timeNow) + ", " + String(temp) + ", " + String(pressure) + ", " + String(depth) + ", " + String(inputP1) + "\r\n"};
//       File dataF = SD.open("/dataLogged.txt", FILE_APPEND);
//       dataF.print(data);
//       // Serial.println(num);
//       bmp_loop();
      
//       // appendFile(SD, "/data.txt", data.c_str());
//       delay(1000); 
//     }
//   }
//   vTaskDelete(Task2);
// }

//Download a file from the SD, it is called in void SD_dir()
void SD_file_download(String filename)
{
  if (SD_present) 
  { 
    File download = SD.open("/"+filename);
    if (download) 
    {
      server.sendHeader("Content-Type", "text/text");
      server.sendHeader("Content-Disposition", "attachment; filename="+filename);
      server.sendHeader("Connection", "close");
      server.streamFile(download, "application/octet-stream");
      download.close();
    } else ReportFileNotPresent("download"); 
  } else ReportSDNotPresent();
}
//Upload a file to the SD
void File_Upload()
{
  // append_page_header();
  webpage = "";
  webpage += F(PAGE_MAIN);
  webpage += F("<h3>Select File to Upload</h3>"); 
  webpage += F("<FORM action='/fupload' method='post' enctype='multipart/form-data'>");
  webpage += F("<input class='buttons' style='width:25%' type='file' name='fupload' id = 'fupload' value=''>");
  webpage += F("<button class='buttons' style='width:10%' type='submit'>Upload File</button><br><br>");
  webpage += F("<a href='/'>[Back]</a><br><br>");
  append_page_footer();
  server.send(200, "text/html", webpage);
}

//Handles the file upload a file to the SD
File UploadFile;
//Upload a new file to the Filing system
void handleFileUpload()
{ 
  HTTPUpload& uploadfile = server.upload(); //See https://github.com/esp8266/Arduino/tree/master/libraries/ESP8266WebServer/srcv
                                            //For further information on 'status' structure, there are other reasons such as a failed transfer that could be used
  if(uploadfile.status == UPLOAD_FILE_START)
  {
    String filename = uploadfile.filename;
    if(!filename.startsWith("/")) filename = "/"+filename;
    Serial.print("Upload File Name: "); Serial.println(filename);
    SD.remove(filename);                         //Remove a previous version, otherwise data is appended the file again
    UploadFile = SD.open(filename, FILE_WRITE);  //Open the file for writing in SD (create it, if doesn't exist)
    filename = String();
  }
  else if (uploadfile.status == UPLOAD_FILE_WRITE)
  {
    if(UploadFile) UploadFile.write(uploadfile.buf, uploadfile.currentSize); // Write the received bytes to the file
  } 
  else if (uploadfile.status == UPLOAD_FILE_END)
  {
    if(UploadFile)          //If the file was successfully created
    {                                    
      UploadFile.close();   //Close the file again
      Serial.print("Upload Size: "); Serial.println(uploadfile.totalSize);
      webpage = "";
      // append_page_header();
      webpage += F(PAGE_MAIN);
      webpage += F("<h3>File was successfully uploaded</h3>"); 
      webpage += F("<h2>Uploaded File Name: "); webpage += uploadfile.filename+"</h2>";
      webpage += F("<h2>File Size: "); webpage += file_size(uploadfile.totalSize) + "</h2><br><br>"; 
      webpage += F("<a href='/'>[Back]</a><br><br>");
      append_page_footer();
      server.send(200,"text/html",webpage);
    } 
    else
    {
      ReportCouldNotCreateFile("upload");
    }
  }
}

// Enabling toggle for data logging
void Data_log()
{
  // webpage = "";
  // append_page_header();
  // webpage += F(PAGE_MAIN);
  if(logData){
    Serial.println("Button press, Stopping logging of data");
  } else {
    Serial.println("Button press, Initiating logging of data");
  }
  logData = !logData;
  server.send(200, "text/html", webpage);
}

void Motor_CW(){
  webpage = "";
  // append_page_header();
  webpage += F(PAGE_MAIN);
  MotorCCW_Switch = 0;
  MotorCW_Switch = 1;
  MotorAll_Disable = 0;
  server.send(200, "text/html", webpage);
}

void Motor_CCW(){
  webpage = "";
  // append_page_header();
  webpage += F(PAGE_MAIN);
  MotorCCW_Switch = 1;
  MotorCW_Switch = 0;
  MotorAll_Disable = 0;
  server.send(200, "text/html", webpage);
}

void Motor_Disable(){
  webpage = "";
  // append_page_header();
  webpage += F(PAGE_MAIN);
  MotorCCW_Switch = 0;
  MotorCW_Switch = 0;
  MotorAll_Disable = 1;
  server.send(200, "text/html", webpage);
}

void Motor_toggle(){
  webpage = "";
  // append_page_header();
  webpage += F(PAGE_MAIN);
  if(MotorToggle){
    Serial.println("Motor press, Stopping Motor");
  } else {
    Serial.println("Motor press, Start Motor CW");
  }
  MotorToggle = !MotorToggle;
  server.send(200, "text/html", webpage);
}

void motorCW(){
  // Set motor direction clockwise
  digitalWrite(enablePin, LOW);
  digitalWrite(dirPin, HIGH);
 
  // Spin motor slowly
  // for(int x = 0; x < stepsPerRevolution; x++)
  // {
    digitalWrite(stepPin, HIGH);
    delayMicroseconds(100);
    digitalWrite(stepPin, LOW);
    delayMicroseconds(100);
  // }
}

void motorCCW(){
  // Set motor direction clockwise
  digitalWrite(enablePin, LOW);
  digitalWrite(dirPin, LOW);
 
  // Spin motor slowly
  // for(int x = 0; x < stepsPerRevolution; x++)
  // {
    digitalWrite(stepPin, HIGH);
    delayMicroseconds(100);
    digitalWrite(stepPin, LOW);
    delayMicroseconds(100);
  // }
}

void motorDisable(){
  digitalWrite(enablePin, HIGH);
}

// code to send the main web page
void SendXML() {
  webpage += F(PAGE_MAIN);
  temp = bmp.readTemperature();
  pressure = bmp.readPressure();
  depth = calc_depth(pressure);
  timeNow = {String(rtc.getHour(h12Flag, pmFlag)) + ":" + String(rtc.getMinute())+ ":" + String(rtc.getSecond())};

  strcpy(XML, "<?xml version = '1.0'?>\n<Data>\n");

  // send bitsA0
  sprintf(buf, "<B0>%d</B0>\n", logData);
  strcat(XML, buf);

  // send bits1
  sprintf(buf, "<B1>%d</B1>\n", motorOn);
  strcat(XML, buf);

  // send temp
  sprintf(buf, "<Temp>%f</Temp>\n", temp);
  strcat(XML, buf);

  // send pressure
  sprintf(buf, "<Pressure>%f</Pressure>\n", pressure);
  strcat(XML, buf);

  // send depth
  sprintf(buf, "<Depth>%f</Depth>\n", depth);
  strcat(XML, buf);
  
  // send time
  // sprintf(buf, "<Time>%s</Time>\n", timeNow);
  // strcat(XML, buf);

  // // show led0 status
  // if (LED0) {
  //   strcat(XML, "<LED>1</LED>\n");
  // }
  // else {
  //   strcat(XML, "<LED>0</LED>\n");
  // }

  // if (SomeOutput) {
  //   strcat(XML, "<SWITCH>1</SWITCH>\n");
  // }
  // else {
  //   strcat(XML, "<SWITCH>0</SWITCH>\n");
  // }

  strcat(XML, "</Data>\n");
  // wanna see what the XML code looks like?
  // actually print it to the serial monitor and use some text editor to get the size
  // then pad and adjust char XML[2048]; above
  Serial.println(XML);

  // you may have to play with this value, big pages need more porcessing time, and hence
  // a longer timeout that 200 ms
  server.send(200, "text/xml", webpage+=XML);
}

//Initial page of the server web, list directory and give you the chance of deleting and uploading
void SD_dir()
{
  webpage = "";
  webpage += F(PAGE_MAIN);
  if (SD_present) 
  {
    //Action acording to post, dowload or delete, by MC 2022
    if (server.args() > 0 ) //Arguments were received, ignored if there are not arguments
    { 
      Serial.println(server.arg(0));
  
      String Order = server.arg(0);
      Serial.println(Order);
      
      if (Order.indexOf("download_")>=0)
      {
        Order.remove(0,9);
        SD_file_download(Order);
        Serial.println(Order);
      }
  
      if ((server.arg(0)).indexOf("delete_")>=0)
      {
        Order.remove(0,7);
        SD_file_delete(Order);
        Serial.println(Order);
      }
    }

    File root = SD.open("/");
    if (root) {
      root.rewindDirectory();
      SendHTML_Header(); 
      webpage += F("<table align='center'>");
      webpage += F("<tr><th>Name/Type</th><th style='width:20%'>Type File/Dir</th><th>File Size</th></tr>");
      printDirectory("/",0);
      webpage += F("</table>");
      SendHTML_Content();
      root.close();
    }
    else 
    {
      SendHTML_Header();
      webpage += F("<h3>No Files Found</h3>");
    }
    // append_page_footer();
    // webpage += F(PAGE_MAIN);
    SendHTML_Content();
    SendHTML_Stop();   //Stop is needed because no content length was sent
  } else ReportSDNotPresent();
}

//Prints the directory, it is called in void SD_dir() 
void printDirectory(const char * dirname, uint8_t levels)
{
  
  File root = SD.open(dirname);

  if(!root){
    return;
  }
  if(!root.isDirectory()){
    return;
  }
  File file = root.openNextFile();

  int i = 0;
  while(file){
    if (webpage.length() > 1000) {
      SendHTML_Content();
    }
    if(file.isDirectory()){
      webpage += "<tr><td>"+String(file.isDirectory()?"Dir":"File")+"</td><td>"+String(file.name())+"</td><td></td></tr>";
      printDirectory(file.name(), levels-1);
    }
    else
    {
      webpage += "<tr><td>"+String(file.name())+"</td>";
      webpage += "<td>"+String(file.isDirectory()?"Dir":"File")+"</td>";
      int bytes = file.size();
      String fsize = "";
      if (bytes < 1024)                     fsize = String(bytes)+" B";
      else if(bytes < (1024 * 1024))        fsize = String(bytes/1024.0,3)+" KB";
      else if(bytes < (1024 * 1024 * 1024)) fsize = String(bytes/1024.0/1024.0,3)+" MB";
      else                                  fsize = String(bytes/1024.0/1024.0/1024.0,3)+" GB";
      webpage += "<td>"+fsize+"</td>";
      webpage += "<td>";
      webpage += F("<FORM action='/' method='post'>"); 
      webpage += F("<button type='submit' name='download'"); 
      webpage += F("' value='"); webpage +="download_"+String(file.name()); webpage +=F("'>Download</button>");
      webpage += "</td>";
      webpage += "<td>";
      webpage += F("<FORM action='/' method='post'>"); 
      webpage += F("<button type='submit' name='delete'"); 
      webpage += F("' value='"); webpage +="delete_"+String(file.name()); webpage +=F("'>Delete</button>");
      webpage += "</td>";
      webpage += "</tr>";

    }
    file = root.openNextFile();
    i++;
  }
  file.close();

 
}

//Delete a file from the SD, it is called in void SD_dir()
void SD_file_delete(String filename) 
{ 
  if (SD_present) { 
    SendHTML_Header();
    File dataFile = SD.open("/"+filename, FILE_READ); //Now read data from SD Card 
    if (dataFile)
    {
      if (SD.remove("/"+filename)) {
        Serial.println(F("File deleted successfully"));
        webpage += "<h3>File '"+filename+"' has been erased</h3>"; 
        webpage += F("<a href='/'>[Back]</a><br><br>");
      }
      else
      { 
        webpage += F("<h3>File was not deleted - error</h3>");
        webpage += F("<a href='/'>[Back]</a><br><br>");
      }
    } else ReportFileNotPresent("delete");
    // append_page_footer(); 
    webpage += F(PAGE_MAIN);
    SendHTML_Content();
    SendHTML_Stop();
  } else ReportSDNotPresent();
} 

//SendHTML_Header
void SendHTML_Header()
{
  server.sendHeader("Cache-Control", "no-cache, no-store, must-revalidate"); 
  server.sendHeader("Pragma", "no-cache"); 
  server.sendHeader("Expires", "-1"); 
  server.setContentLength(CONTENT_LENGTH_UNKNOWN); 
  server.send(200, "text/html", ""); //Empty content inhibits Content-length header so we have to close the socket ourselves. 
  // append_page_header();
  server.sendContent(webpage);
  webpage = "";
}

//SendHTML_Content
void SendHTML_Content()
{
  server.sendContent(webpage);
  webpage = "";
}

//SendHTML_Stop
void SendHTML_Stop()
{
  server.sendContent("");
  server.client().stop(); //Stop is needed because no content length was sent
}

/* //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SelectInput(String heading1, String command, String arg_calling_name)
{
  SendHTML_Header();
  webpage += F("<h3>"); webpage += heading1 + "</h3>"; 
  webpage += F("<FORM action='/"); webpage += command + "' method='post'>"; // Must match the calling argument e.g. '/chart' calls '/chart' after selection but with arguments!
  webpage += F("<input type='text' name='"); webpage += arg_calling_name; webpage += F("' value=''><br>");
  webpage += F("<type='submit' name='"); webpage += arg_calling_name; webpage += F("' value=''><br><br>");
  append_page_footer();
  SendHTML_Content();
  SendHTML_Stop();
} */

//ReportSDNotPresent
void ReportSDNotPresent()
{
  SendHTML_Header();
  webpage += F("<h3>No SD Card present</h3>"); 
  webpage += F("<a href='/'>[Back]</a><br><br>");
  append_page_footer();
  SendHTML_Content();
  SendHTML_Stop();
}

//ReportFileNotPresent
void ReportFileNotPresent(String target)
{
  SendHTML_Header();
  webpage += F("<h3>File does not exist</h3>"); 
  webpage += F("<a href='/"); webpage += target + "'>[Back]</a><br><br>";
  append_page_footer();
  SendHTML_Content();
  SendHTML_Stop();
}

//ReportCouldNotCreateFile
void ReportCouldNotCreateFile(String target)
{
  SendHTML_Header();
  webpage += F("<h3>Could Not Create Uploaded File (write-protected?)</h3>"); 
  webpage += F("<a href='/"); webpage += target + "'>[Back]</a><br><br>";
  append_page_footer();
  SendHTML_Content();
  SendHTML_Stop();
}

//File size conversion
String file_size(int bytes)
{
  String fsize = "";
  if (bytes < 1024)                 fsize = String(bytes)+" B";
  else if(bytes < (1024*1024))      fsize = String(bytes/1024.0,3)+" KB";
  else if(bytes < (1024*1024*1024)) fsize = String(bytes/1024.0/1024.0,3)+" MB";
  else                              fsize = String(bytes/1024.0/1024.0/1024.0,3)+" GB";
  return fsize;
}

